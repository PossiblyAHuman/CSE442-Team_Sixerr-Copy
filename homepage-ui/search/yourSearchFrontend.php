...
if(numba == 1 || numba == 2 || numba == 5)
{
    var array = [
        <?php
            ...
            {
                $stmt = $conn->prepare("SELECT * FROM posts WHERE ((title LIKE ?) AND (user_id = ?)) OR ((description like ?) AND (user_id = ?))");

                $query = "%$query%";

                $stmt->bind_param('ssss', $query, $id, $query, $id);
                $stmt->execute();
                $raw_results = $stmt->get_result();
...
else if(numba == 3 || numba == 4)
{
    var array = [
        ...
        if(strlen($query) == 0)
        {
            $stmt = $conn->prepare("SELECT * FROM posts WHERE user_id  = ? ORDER BY price ASC");
            $stmt->bind_param('s', $id);
            $stmt->execute();
            $raw_results = $stmt->get_result();
        ...
        {
            $stmt = $conn->prepare("SELECT * FROM posts WHERE (((title LIKE ?) AND (user_id = ?)) OR ((description like ?) AND (user_id = ?))) ORDER BY price ASC");
            $query = "%$query%";
